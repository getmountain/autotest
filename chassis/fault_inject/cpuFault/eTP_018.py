...
'''
用例名称: 注入低温TP点，产生故障后恢复
前置步骤：所有docker均已启动。roscore已运行
测试步骤：1、注入低温TP点
          //产生正确告警，power_alarm为2
          2、取消TP点
          //告警恢复，power_alarm为0   
作者:Li Siying
创建时间：2020-01-09
...
class Test_eTP_018():
    ...
    def teardown_method(self):
        logging.info('---------after-----------')
        tp_main_set(self.container, '18', '0')

    # 测试步骤
    def test_eTP_018(self):
        logpath = os.getcwd().replace('autotest', 'auto_result/log') + '/' + os.path.basename(__file__)
        logging.info('前置步骤：所有docker均已启动。roscore已运行')

        logging.info('1、注入低温TP点'
                     '//产生正确告警，power_alarm为2')
        tp_main_set(self.container,'18','1')
        result = chassis_alarm_echo(self.container)
        if result['power_alarm'] != '2':
