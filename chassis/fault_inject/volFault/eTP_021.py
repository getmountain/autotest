...
class Test_eTP_021():
    ...
    def teardown_method(self):
        logging.info('---------after-----------')
        tp_main_set(self.container, '21', '0')
    # 测试步骤
    def test_eTP_021(self):

        logging.info('1、注入低电压TP点'
                     '//产生正确告警，power_alarm为16')
        tp_main_set(self.container,'21','1')
        result = chassis_alarm_echo(self.container)
        if result['power_alarm'] != '16':
