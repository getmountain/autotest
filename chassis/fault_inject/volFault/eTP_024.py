...
'''
用例名称: 注入超低电压TP点，产生故障后恢复
前置步骤：所有docker均已启动。roscore已运行
测试步骤：1、注入超低电压TP点
          //产生正确告警，power_alarm为2048
          2、取消TP点
          //告警恢复，power_alarm为0   
作者:Li Siying
创建时间：2019-06-27
...
class Test_eTP_024():
    ...
    def teardown_method(self):
        logging.info('---------after-----------')
        tp_main_set(self.container, '24', '0')
    # 测试步骤
    def test_eTP_024(self):
        logging.info('1、注入超低电压TP点'
                     '//产生正确告警，power_alarm为16')
        tp_main_set(self.container,'24','1')
        result = chassis_alarm_echo(self.container)
        if result['power_alarm'] != '2048':
