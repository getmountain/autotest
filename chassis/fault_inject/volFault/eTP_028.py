...
'''
用例名称: 注入12V内部电压异常TP点，产生故障后恢复
前置步骤：所有docker均已启动。roscore已运行
测试步骤：1、注入12V内部电压异常TP点
          //产生正确告警，power_alarm为4096（bit12）
          2、取消TP点
          //告警恢复，power_alarm为0   
作者:Li Siying
创建时间：2019-06-27
...
class Test_eTP_028():
    ...
    def teardown_method(self):
        logging.info('---------after-----------')
        tp_main_set(self.container, '28', '0')
    # 测试步骤
    def test_eTP_028(self):
        logging.info('1、注入12V内部电压异常TP点'
                     '//产生正确告警，power_alarm为4096')
        tp_main_set(self.container,'28','1')
        result = chassis_alarm_echo(self.container)
        if result['power_alarm'] != '4096':
