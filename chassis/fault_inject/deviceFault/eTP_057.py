...
'''
用例名称: 注入电源板与bma通讯断开标识TP点，产生故障后恢复
前置步骤：所有docker均已启动。roscore已运行
测试步骤：1、注入电源板与bma通讯断开标识TP点
          //产生正确告警，power_alarm为64
          2、取消TP点
          //告警恢复，power_alarm为0
作者:Li Siying
创建时间：2019-08-06
...
class Test_eTP_057():
    ...
    def teardown_method(self):
        logging.info('---------after-----------')
        tp_main_set(self.container, '57', '0')
    # 测试步骤
    def test_eTP_057(self):
        logging.info('1、注入电源板与bma通讯断开标识TP点'
                     '//产生正确告警，power_alarm为64')
        tp_main_set(self.container,'57','1')
        result = chassis_alarm_echo(self.container)
        if result['power_alarm'] != '64':
