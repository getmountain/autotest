...
'''
用例名称: 注入摇杆标定错误TP点，产生故障后恢复
前置步骤：所有docker均已启动。roscore已运行
测试步骤：1、注入摇杆标定错误TP点
          //产生正确告警，external_sensor_alram为524288 （bit19）
          2、取消TP点
...
class Test_eTP_063():
    ...
    def teardown_method(self):
        logging.info('---------after-----------')
        tp_main_set(self.container, '63', '0')
    # 测试步骤
    def test_eTP_063(self):
        logging.info('1、注入摇杆标定错误TP点'
                     '//产生正确告警，external_sensor_alram为524288')
        tp_main_set(self.container,'63','1')
        result = chassis_alarm_echo(self.container)
        if result['external_sensor_alarm'] != '524288':
