...
'''
用例名称: 注入imu硬件/通讯错误TP点，产生故障后恢复
前置步骤：所有docker均已启动。roscore已运行
测试步骤：1、注入imu硬件/通讯错误TP点
          //产生正确告警，internal_sensor_alarm为8，/chassis_state状态imu为8
          2、取消TP点
...
class Test_eTP_032():
    ...
    def teardown_method(self):
        logging.info('---------after-----------')
        tp_main_set(self.container, '32', '0')
    # 测试步骤
    def test_eTP_032(self):
        logging.info('1、注入imu硬件/通讯错误TP点'
                     '//产生正确告警，internal_sensor_alarm为8，/chassis_state状态imu为2')
        tp_main_set(self.container,'32','1')
        result = chassis_alarm_echo(self.container)
        if result['internal_sensor_alarm'] != '8':
