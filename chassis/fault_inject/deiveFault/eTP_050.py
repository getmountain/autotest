...
'''
用例名称: 注入左驱动器启动时加载默认配置故障TP点，产生故障后恢复
前置步骤：所有docker均已启动。roscore已运行
测试步骤：1、注入左驱动器启动时加载默认配置故障TP点
          //产生正确告警，motor_driver_alarm为8388608（bit23）
          2、取消TP点
...
class Test_eTP_050():
    ...
    def teardown_method(self):
        logging.info('---------after-----------')
        tp_main_set(self.container, '50', '0')
    # 测试步骤
    def test_eTP_050(self):
        logging.info('1、注入左驱动器启动时加载默认配置故障TP点'
                     '//产生正确告警，motor_driver_alarm为8388608')
        tp_main_set(self.container,'50','1')
        result = chassis_alarm_echo(self.container)
        if result['motor_driver_alarm'] != '8388608':
