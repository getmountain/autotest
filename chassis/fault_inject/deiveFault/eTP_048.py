...
'''
用例名称: 注入左驱动器无刷传感器故障TP点，产生故障后恢复
前置步骤：所有docker均已启动。roscore已运行
测试步骤：1、注入左驱动器无刷传感器故障TP点
          //产生正确告警，motor_driver_alarm为2097152（bit21）
          2、取消TP点
...
class Test_eTP_048():
    ...
    def teardown_method(self):
        logging.info('---------after-----------')
        tp_main_set(self.container, '48', '0')
    # 测试步骤
    def test_eTP_048(self):
        logging.info('1、注入左驱动器无刷传感器故障TP点'
                     '//产生正确告警，motor_driver_alarm为2097152')
        tp_main_set(self.container,'48','1')
        result = chassis_alarm_echo(self.container)
        if result['motor_driver_alarm'] != '2097152':
