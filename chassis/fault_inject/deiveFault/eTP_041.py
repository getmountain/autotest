...
'''
用例名称: 注入右驱动器MOSFET故障TP点，产生故障后恢复
前置步骤：所有docker均已启动。roscore已运行
测试步骤：1、注入右驱动器MOSFET故障TP点
          //产生正确告警，motor_driver_alarm为16384（bit14）
          2、取消TP点
...
class Test_eTP_041():
    ...
    def teardown_method(self):
        logging.info('---------after-----------')
        tp_main_set(self.container, '41', '0')
    # 测试步骤
    def test_eTP_041(self):
        logging.info('1、注入右驱动器MOSFET故障TP点'
                     '//产生正确告警，motor_driver_alarm为16384')
        tp_main_set(self.container,'41','1')
        result = chassis_alarm_echo(self.container)
        if result['motor_driver_alarm'] != '16384':
