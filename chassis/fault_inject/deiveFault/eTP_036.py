...
'''
用例名称: 注入右驱动器过压TP点，产生故障后恢复
前置步骤：所有docker均已启动。roscore已运行
测试步骤：1、注入右驱动器过压TP点
          //产生正确告警，motor_driver_alarm为512（bit9）
          2、取消TP点
...
class Test_eTP_036():
    ...
    def teardown_method(self):
        logging.info('---------after-----------')
        tp_main_set(self.container, '36', '0')
    # 测试步骤
    def test_eTP_036(self):
        logging.info('1、注入右驱动器过压TP点'
                     '//产生正确告警，motor_driver_alarm为512')
        tp_main_set(self.container,'36','1')
        result = chassis_alarm_echo(self.container)
        if result['motor_driver_alarm'] != '512':
