...
'''
用例名称: 注入左驱动器短路TP点，产生故障后恢复
前置步骤：所有docker均已启动。roscore已运行
测试步骤：1、注入左驱动器短路TP点
          //产生正确告警，motor_driver_alarm为524288（bit19）
          2、取消TP点
...
class Test_eTP_046():
    ...
    def teardown_method(self):
        logging.info('---------after-----------')
        tp_main_set(self.container, '46', '0')
    # 测试步骤
    def test_eTP_046(self):
        logging.info('1、注入左驱动器短路TP点'
                     '//产生正确告警，motor_driver_alarm为524288')
        tp_main_set(self.container,'46','1')
        result = chassis_alarm_echo(self.container)
        if result['motor_driver_alarm'] != '524288':
