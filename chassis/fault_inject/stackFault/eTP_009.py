...
'''
用例名称: 注入TP点，使通讯任务剩余堆栈量达到告警值
前置步骤：所有docker均已启动。roscore已运行
测试步骤：1、注入9号TP点，模拟通讯任务剩余堆栈量为49
          //产生通讯堆栈异常告警system_alarm为2，下位机状态显示堆栈剩余量为49
          2、取消TP点
...
class Test_eTP_009():
    ...
    def teardown_method(self):
        logging.info('---------after-----------')
        tp_main_set(self.container, '9', '0')
    # 测试步骤
    def test_eTP_009(self):
        logging.info('1、注入9号TP点，模拟通讯任务剩余堆栈量为49'
                     '//产生通讯堆栈异常告警system_alarm为4，下位机状态显示堆栈剩余量为49')
        # 检查告警为bit2
        tp_main_set(self.container,'9','1')
        result = chassis_alarm_echo(self.container)
        if result['system_alarm'] != '4':
            raise Exception('告警不正确')

        # 检查堆栈剩余量
        result = chassis_state_echo(self.container)
        if result['stack_comm'] != '49':
