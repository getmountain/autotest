...
import time
'''
用例名称: 运营故障时，发生的不可驾驶故障部分恢复，状态不切换
...
测试步骤：1、切换运营模式
          ...
          3、注入platform_datacollect_sensor_state故障,ID 65541
          //告警sensor_state collect failed正确
          4、取消注入的故障ID 65540
          //该告警恢复，状态机仍为维护故障态
作者:Li Siying
创建时间：2019-08-12
...
def State_change028():
    ...
    logging.info('2、注入platform_datacollect_sensor_state故障,id65541'
                 '//上位机进入故障态，下位机进入故障态，告警sensor_state collect failed正确')
    ...
    logging.info('3、取消注入的故障ID 65540'
                 '//该告警恢复，状态机仍为维护故障态')
    ...
    # 取消platform_datacollect_sensor_state故障注入
